...
(() => {
  ...
  // Show the menu if the user tabs to the links in it
  document.addEventListener('keyup', (e) => {
    const target = e.target;
    if (target.classList.contains('menu__link')) {
      const key = e.key;
      if (key === 'Tab' && !menu.classList.contains('menu--show')) {
        menu.classList.add('menu--show');
        button.classList.add('navbar__hamburger-icon--cross');
      }
    } else {
      if (menu.classList.contains('menu--show')) {
        menu.classList.remove('menu--show');
        button.classList.remove('navbar__hamburger-icon--cross');
      }
    }
  });
})();
...
(() => {
  ...
  function aboutMeRestructure(mediaQuery) {
    if (mediaQuery.matches) {
      ...
      document
        .querySelector('#aboutMeDesktopContainer')
        .classList.add('text-title-container');
    } else {
  ...
  skillArray.forEach(function (skill) {
    skill.addEventListener('click', (e) => {
      if (!desktopQuery.matches) {
        ...
        if (!expanded) {
          temp.parentNode.getElementsByTagName('p')[0].style.maxHeight =
            temp.parentNode.getElementsByTagName('p')[0].scrollHeight + 'px';
          expanded = true;
        } else {
